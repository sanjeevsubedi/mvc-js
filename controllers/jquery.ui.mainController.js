...
(function ($) {
    ...
    $.widget("ui.mainController", {
        ...
        */
        ...
        _calculatePedingSurveys:function(){
            
        	databaseHelper.execQuery("select * from RESPONSE where status=?", [1], function (results, err) {
                                     if (results) {
                                     var counts = results.rows.length;
                                     var pendElem = $("#pendingResponses");
                                     if(counts > 0) {
                                    	 pendElem.removeClass("green-banner");
                                    	 pendElem.addClass("red-banner");
                                    	 pendElem.html(langMgr.getTranslation("PendMsgCount",[counts]));
